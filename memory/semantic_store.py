...
Concern 4 Support:
Stores structured semantic facts (e.g. researcher clearance levels, payload risk classifications,
site policy overrides, synthesis clearance rules) extracted exclusively via the consolidation layer.

Features:
- Versioning: Keeps historical versions with valid_from / valid_until timestamps.
- Expiration: Handles expired facts (is_active = 0).
- Contradiction Traceability: Preserves superseded facts rather than silently overwriting them.
"""
...
class SemanticStore:
    ...
    def add_or_update_fact(
        ...
        If supersede_existing is True and an active fact exists for key (entity_type:entity_id:attribute):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if existing and supersede_existing:
                ...
                # Supersede existing version (never overwrite silently!)
                cursor.execute(
